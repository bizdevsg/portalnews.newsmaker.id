@section('namePage', 'Analisis Pasar Indonesia')
...
<x-app-layout>
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="space-y-2">
                    ...
                    <h1 class="text-3xl font-bold text-slate-900 dark:text-slate-100">Analisis</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">
                        Kelola analisis Pasar Indonesia dengan pola input bilingual seperti Newsmaker 23 tanpa kategori.
                    </p>
                </div>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <a
                        href="{{ route('pasar-indonesia.analisis.create') }}"
        ...
        <section class="mt-6">
            @if ($items->isEmpty())
                <div class="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center shadow-sm dark:border-slate-700 dark:bg-slate-900">
                    ...
                    <p class="mt-4 text-sm text-slate-500 dark:text-slate-400">Belum ada analisis.</p>
                </div>
            @else
                    ...
                    @foreach ($items as $item)
                        <article class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
                            ...
                            <div class="p-5">
                                ...
                                <form id="delete-form-{{ $item->id }}" action="{{ route('pasar-indonesia.analisis.destroy', $item->id) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                                ...
                                <div class="mt-5 grid grid-cols-3 gap-3">
                                    <a
                                        href="{{ route('pasar-indonesia.analisis.show', $item->id) }}"
                                        class="inline-flex items-center justify-center rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
                                        Preview
                                    </a>
                                    <a
                                        href="{{ route('pasar-indonesia.analisis.edit', $item->id) }}"
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/50 px-4" aria-hidden="true">
        <div class="w-full rounded-2xl border border-slate-200 bg-white p-6 shadow-xl dark:border-slate-800 dark:bg-slate-900 md:w-[28rem]">
            <h2 class="text-lg font-bold text-slate-900 dark:text-slate-100">Hapus Analisis</h2>
