@section('namePage', 'Tambah Berita Regulasi & Institusi')

<x-app-layout>
    @php
        $backUrl = route('regulasi-institusi.kategori.show', $selectedCategory->slug);
    @endphp

    <form id="newsForm" action="{{ route('regulasi-institusi.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="grid grid-cols-1 gap-5 xl:grid-cols-3">
                        <div>
                            ...
                            <input type="file" id="image" name="image" accept="image/*"
                                class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-2 text-slate-900 outline-none transition file:mr-2 file:rounded-xl file:border-0 file:bg-slate-900 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-white hover:file:bg-slate-800 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 @error('image') border-rose-500 @enderror"
                                required>
                            @error('image')
                                <p class="mt-2 text-sm font-medium text-rose-600 dark:text-rose-300">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" id="author_display" value="{{ auth()->user()->name }}"
