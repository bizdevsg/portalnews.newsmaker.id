...
<x-app-layout>
    <div class="w-full px-4 py-8 sm:px-6 lg:px-8">
        <section
            ...
            <div class="flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="flex flex-wrap gap-3">
                    <a href="{{ route('regulasi-institusi.create', ['category_id' => $category->id]) }}"
        ...
        @if (session('success'))
            <div id="successAlert" data-flash-alert
                class="mt-6 flex items-center justify-between gap-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-emerald-700 shadow-sm dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-200">
                <div class="flex items-center gap-3">
                    <i class="fa-solid fa-circle-check"></i>
                    <p class="text-sm font-medium leading-6">{{ session('success') }}</p>
                </div>
                <button type="button" id="closeSuccessAlert" data-flash-alert-close
                        ...
                        <article
                            ...
                            <div class="p-5">
                                ...
                                <div class="mt-4 space-y-2 text-sm text-slate-600 dark:text-slate-300">
                                    <p><span class="font-semibold text-slate-900 dark:text-slate-100">Author:</span>
                                        {{ $item->author?->name ?? '-' }}</p>
                                    <p><span class="font-semibold text-slate-900 dark:text-slate-100">Source:</span>
                                        {{ $item->source }}</p>
                                </div>

                                <form id="delete-article-form-{{ $item->id }}"
                                    action="{{ route('regulasi-institusi.destroy', $item->id) }}" method="POST"
                                    class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                                ...
                                <div class="mt-5 grid grid-cols-3 gap-3">
                                    <a href="{{ route('regulasi-institusi.show', $item->id) }}"
                                        class="inline-flex items-center justify-center rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
                                        Preview
                                    </a>
                                    <a href="{{ route('regulasi-institusi.edit', $item->id) }}"
    ...
    <div id="deleteArticleModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/50 px-4"
        aria-hidden="true">
        <div
            ...
            <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">
                Artikel <span id="deleteArticleName" class="font-semibold text-slate-900 dark:text-slate-100"></span>
                akan dihapus.
            </p>

            <div class="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                <button type="button" id="cancelDeleteArticleButton"
                    class="inline-flex items-center justify-center rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
                    Batal
                </button>
                <button type="button" id="confirmDeleteArticleButton"
