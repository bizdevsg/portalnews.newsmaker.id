@section('namePage', 'Preview Berita Pasar Indonesia')

<x-app-layout>
    @php
        $backUrl = $item->categoryItem
            ? route('pasar-indonesia.berita.kategori.show', $item->categoryItem->slug)
            : route('pasar-indonesia.berita.index');
    @endphp

    <div class="w-full px-4 py-8 sm:px-6 lg:px-8">
        <section class="overflow-hidden rounded-[32px] bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-900 px-6 py-8 shadow-2xl ring-1 ring-white/10 sm:px-8 lg:px-10">
            <div class="flex flex-wrap items-start justify-between gap-6">
                <div class="space-y-4">
                    <span class="inline-flex items-center rounded-full border border-white/10 bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.22em] text-emerald-100">
                        Preview Berita
                    </span>
                    <div class="space-y-2">
                        <p class="text-xs font-semibold uppercase tracking-[0.32em] text-slate-300">
                            {{ $item->categoryItem?->name ?? 'Pasar Indonesia' }}
                        </p>
                        <h1 class="text-3xl font-bold tracking-tight text-white sm:text-4xl">{{ $item->title_id }}</h1>
                        <p class="text-sm leading-6 text-emerald-100 sm:text-base">{{ $item->title_en }}</p>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('pasar-indonesia.berita.edit', $item->id) }}" class="inline-flex items-center justify-center rounded-xl bg-white px-5 py-3 text-sm font-semibold text-slate-900 transition hover:bg-emerald-50">
                        Edit Berita
                    </a>
                </div>
            </div>
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div class="rounded-2xl bg-slate-50 p-4 dark:bg-slate-950">
                            ...
                            <p class="mt-2 text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $item->categoryItem?->name ?? $item->category_label ?? '-' }}</p>
                        </div>
                        <div class="rounded-2xl bg-slate-50 p-4 dark:bg-slate-950">
                            <p class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">Author</p>
                            <p class="mt-2 text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $item->author_initial ?? $item->author?->name ?? '-' }}</p>
                        </div>
                        <div class="rounded-2xl bg-slate-50 p-4 dark:bg-slate-950">
