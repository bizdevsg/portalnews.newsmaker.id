<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="card p-6 bg-white rounded shadow">
            <form id="pivotForm" action="{{ route('ebook.store') }}" method="POST" enctype="multipart/form-data"
                class="space-y-4">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="file" name="cover_image" id="cover_image"
                            class="w-full border rounded p-2 @error('cover_image') border-red-500 @enderror" required>
                        @error('cover_image')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="file" name="file_ebook" id="file_ebook"
                            class="w-full border rounded p-2 @error('file_ebook') border-red-500 @enderror" required>
                        @error('file_ebook')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
    ...
    <div id="submitModal"
        class="fixed inset-0 bg-black/50 backdrop-blur flex items-center justify-center hidden z-50 px-5">
        <div class="bg-white rounded p-6 w-full max-w-md">
            ...
            <div class="flex justify-end gap-4">
                ...
                <button type="submit" form="pivotForm" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Ya,
                    Simpan</button>
            </div>
        </div>
    </div>
