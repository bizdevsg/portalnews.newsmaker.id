@section('namePage', 'Tambah Analisis Pasar Indonesia')

<x-app-layout>
    <form id="analysisForm" action="{{ route('pasar-indonesia.analisis.store') }}" method="POST" enctype="multipart/form-data" class="w-full px-4 py-8 sm:px-6 lg:px-8">
        @csrf
            ...
            <div class="flex flex-wrap items-start justify-between gap-6">
                <div class="space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <h1 class="text-3xl font-bold tracking-tight text-white sm:text-4xl">Buat analisis dua bahasa</h1>
        ...
        @if ($errors->any())
            <div class="mt-6 rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 text-sm text-rose-700 shadow-sm dark:border-rose-500/20 dark:bg-rose-500/10 dark:text-rose-200">
                <p class="font-semibold">Masih ada field analisis yang perlu diperbaiki.</p>
            </div>
        @endif
        ...
        <div class="mt-8 space-y-6">
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-6">
                        <div class="rounded-[24px] border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/40">
                            ...
                            <textarea id="content_id" name="content_id" rows="10" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 @error('content_id') border-rose-500 @enderror" placeholder="Masukkan isi analisis Indonesia...">{{ old('content_id') }}</textarea>
                            @error('content_id')
                                <p class="mt-2 text-sm font-medium text-rose-600 dark:text-rose-300">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="rounded-[24px] border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/40">
                            ...
                            <textarea id="content_en" name="content_en" rows="10" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 @error('content_en') border-rose-500 @enderror" placeholder="Masukkan isi analisis Inggris...">{{ old('content_en') }}</textarea>
                            @error('content_en')
                                <p class="mt-2 text-sm font-medium text-rose-600 dark:text-rose-300">{{ $message }}</p>
                            @enderror
                        </div>
            ...
            <div class="flex flex-wrap justify-end gap-3">
                <a href="{{ route('pasar-indonesia.analisis.index') }}" class="inline-flex items-center justify-center rounded-xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800">
                    Kembali
                </a>
