@section('namePage', 'Edit Analisis Pasar Indonesia')

<x-app-layout>
    <form id="analysisForm" action="{{ route('pasar-indonesia.analisis.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="w-full px-4 py-8 sm:px-6 lg:px-8">
        @csrf
        @method('PUT')
            ...
            <div class="flex flex-wrap items-start justify-between gap-6">
                <div class="space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <h1 class="text-3xl font-bold tracking-tight text-white sm:text-4xl">Perbarui analisis dua bahasa</h1>
                        <p class="text-sm leading-6 text-indigo-100 sm:text-base">
                            Edit media, judul, source, dan konten bilingual pada analisis Pasar Indonesia.
                        </p>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-6">
                        <div class="rounded-[24px] border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/40">
                            ...
                            <textarea id="content_id" name="content_id" rows="10" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none transition focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 @error('content_id') border-rose-500 @enderror" placeholder="Masukkan isi analisis Indonesia...">{{ old('content_id', $item->content_id) }}</textarea>
                            @error('content_id')
                                <p class="mt-2 text-sm font-medium text-rose-600 dark:text-rose-300">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="rounded-[24px] border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/40">
                            ...
                            <textarea id="content_en" name="content_en" rows="10" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none transition focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 @error('content_en') border-rose-500 @enderror" placeholder="Masukkan isi analisis Inggris...">{{ old('content_en', $item->content_en) }}</textarea>
                            @error('content_en')
                                <p class="mt-2 text-sm font-medium text-rose-600 dark:text-rose-300">{{ $message }}</p>
                            @enderror
                        </div>
