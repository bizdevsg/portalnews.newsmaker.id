@section('namePage', 'Kategori Berita Pasar Indonesia')
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="space-y-2">
                    ...
                    <h1 class="text-3xl font-bold text-slate-900 dark:text-slate-100">Kategori Berita</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">
                        Kelola kategori untuk mengelompokkan berita Pasar Indonesia. Tambah berita dilakukan dari halaman detail kategori.
                    </p>
                </div>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <a
                        href="{{ route('pasar-indonesia.berita.kategori.create') }}"
        ...
        <section class="mt-6">
            @if ($categories->isEmpty())
                <div class="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center shadow-sm dark:border-slate-700 dark:bg-slate-900">
                    ...
                    <p class="mt-4 text-sm text-slate-500 dark:text-slate-400">Belum ada kategori berita.</p>
                </div>
            @else
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
                    @foreach ($categories as $category)
                        <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                            ...
                            <form id="delete-category-form-{{ $category->id }}" action="{{ route('pasar-indonesia.berita.kategori.destroy', $category->id) }}" method="POST" class="hidden">
                                @csrf
                                @method('DELETE')
                            </form>
                            ...
                            <div class="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
                                <a
                                    href="{{ route('pasar-indonesia.berita.kategori.show', $category->slug) }}"
