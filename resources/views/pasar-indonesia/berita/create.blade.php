...
<x-app-layout>
    @php
        $backUrl = route('pasar-indonesia.berita.kategori.show', $selectedCategory->slug);
    @endphp

    <form id="newsForm" action="{{ route('pasar-indonesia.berita.store') }}" method="POST" enctype="multipart/form-data"
        class="w-full px-4 py-8 sm:px-6 lg:px-8">
        @csrf
        <input type="hidden" name="category_id" value="{{ old('category_id', $selectedCategory->id) }}">
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <p class="text-sm leading-6 text-blue-100 sm:text-base">
                            Anda sedang menambahkan berita ke kategori {{ $selectedCategory->name }}. Lengkapi meta
                            artikel, judul Indonesia dan Inggris, serta konten editor untuk kedua bahasa.
                        </p>
                    </div>
                </div>
            ...
            <section
                ...
                <div class="grid grid-cols-1 gap-6 xl:grid-cols-[1.05fr_0.95fr]">
                    ...
                    <div>
                        ...
                        <div
                            class="rounded-2xl border border-slate-300 bg-slate-50 px-4 py-4 dark:border-slate-700 dark:bg-slate-950">
                            <p class="text-base font-semibold text-slate-900 dark:text-slate-100">
                                {{ $selectedCategory->name }}
                            </p>
                            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                                Jika ingin menambah berita ke kategori lain, kembali dulu ke halaman kategori lalu buka
                                kategori tujuan.
                            </p>
                        </div>
                        @error('category_id')
                    ...
                    <div class="grid grid-cols-1 gap-5 xl:grid-cols-3">
                        <div>
                            ...
                            <input type="file" id="image" name="image" accept="image/*"
                                ...
                                required>
                            <img id="image-preview" alt="Preview gambar utama"
                                class="mt-4 hidden h-40 w-full rounded-2xl object-cover">
                            @error('image')
                                <p class="mt-2 text-sm font-medium text-rose-600 dark:text-rose-300">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            @php
                                ...
                                $currentAuthor = old('author', strtoupper((string) auth()->user()->username));
                            @endphp
                            <select id="author" name="author"
                                class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 @error('author') border-rose-500 @enderror"
                                required>
                        ...
                        <div>
                            ...
                            <input type="text" id="source" name="source" value="{{ old('source') }}"
                                ...
                                class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 @error('source') border-rose-500 @enderror"
                                required>
                            @error('source')
                                <p class="mt-2 text-sm font-medium text-rose-600 dark:text-rose-300">{{ $message }}</p>
                            @enderror
