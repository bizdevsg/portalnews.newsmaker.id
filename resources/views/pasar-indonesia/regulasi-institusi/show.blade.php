@section('namePage', 'Preview Regulasi & Institusi')

<x-app-layout>
    @php
        $backUrl = $item->categoryItem
            ? route('regulasi-institusi.kategori.show', $item->categoryItem->slug)
        ...
        <section class="overflow-hidden rounded-[32px] bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-900 px-6 py-8 shadow-2xl ring-1 ring-white/10 sm:px-8 lg:px-10">
            <div class="flex flex-wrap items-start justify-between gap-6">
                <div class="space-y-4">
                    ...
                    <div class="space-y-2">
                        <p class="text-xs font-semibold uppercase tracking-[0.32em] text-slate-300">
                            {{ $item->categoryItem?->name ?? 'Regulasi & Institusi' }}
                        </p>
                        <h1 class="text-3xl font-bold tracking-tight text-white sm:text-4xl">{{ $item->title_id }}</h1>
                        <p class="text-sm leading-6 text-indigo-100 sm:text-base">{{ $item->title_en }}</p>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('regulasi-institusi.edit', $item->id) }}" class="inline-flex items-center justify-center rounded-xl bg-white px-5 py-3 text-sm font-semibold text-slate-900 transition hover:bg-indigo-50">
                        Edit Berita
                    </a>
                </div>
            </div>
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div class="rounded-2xl bg-slate-50 p-4 dark:bg-slate-950">
                            ...
                            <p class="mt-2 text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $item->categoryItem?->name ?? '-' }}</p>
                        </div>
                        <div class="rounded-2xl bg-slate-50 p-4 dark:bg-slate-950">
