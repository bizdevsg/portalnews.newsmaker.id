@section('namePage', 'Edit Pop Up Banner')

<x-app-layout>
    <form action="{{ route('popup-banner.update', $popupBanner->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="flex flex-wrap items-start justify-between gap-6">
                <div class="space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <h1 class="text-3xl font-bold tracking-tight text-white sm:text-4xl">Perbarui popup banner</h1>
                        <p class="text-sm leading-6 text-blue-100 sm:text-base">
                            Ubah banner <span class="font-semibold text-white">{{ $popupBanner->title }}</span> tanpa
                            mengubah alur data yang sudah ada, termasuk kalau design modalnya memakai HTML kustom.
                        </p>
                    </div>
                </div>
        ...
        @if ($errors->any())
            <div
                ...
                <p class="font-semibold">Periksa perubahan popup banner sebelum disimpan.</p>
            </div>
        @endif

        @include('popup-banner.partials.form-fields', ['popupBanner' => $popupBanner])
    </form>
</x-app-layout>
