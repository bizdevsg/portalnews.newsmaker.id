@section('namePage', 'Kategori Regulasi & Institusi')
...
<x-app-layout>
    <div class="w-full px-4 py-8 sm:px-6 lg:px-8">
        <section
            ...
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div class="space-y-2">
                    ...
                    <h1 class="text-3xl font-bold text-slate-900 dark:text-slate-100">Kategori Regulasi & Institusi</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">
                        Kelola kategori berita untuk modul Regulasi & Institusi. Tambah berita dilakukan dari halaman
                        detail kategori.
                    </p>
                </div>

                <div class="flex flex-wrap gap-3">
                    <a href="{{ route('pasar-indonesia.index') }}"
                    ...
                    <a href="{{ route('regulasi-institusi.kategori.create') }}"
        ...
        @if (session('info'))
            <div id="infoAlert"
                class="mt-6 flex items-start justify-between gap-4 rounded-2xl border border-blue-200 bg-blue-50 px-5 py-4 text-blue-700 shadow-sm dark:border-blue-500/20 dark:bg-blue-500/10 dark:text-blue-200">
                <div class="flex items-start gap-3">
                    <i class="fa-solid fa-circle-info mt-0.5"></i>
                    <p class="text-sm font-medium leading-6">{{ session('info') }}</p>
                </div>
                <button type="button" id="closeInfoAlert"
                    class="inline-flex h-9 w-9 items-center justify-center rounded-full transition hover:bg-blue-100 dark:hover:bg-blue-500/10">
                    <i class="fa-solid fa-xmark"></i>
                </button>
            </div>
        @endif
        ...
        <section class="mt-6">
            @if ($categories->isEmpty())
                <div
                    class="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center shadow-sm dark:border-slate-700 dark:bg-slate-900">
                    <img src="{{ asset('assets/hand-drawn-no-data-concept.png') }}" alt="No Data"
                        class="mx-auto h-40 object-contain">
                    <p class="mt-4 text-sm text-slate-500 dark:text-slate-400">Belum ada kategori Regulasi & Institusi.
                    </p>
                </div>
            @else
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
                    @foreach ($categories as $category)
                        <article
                            ...
                            <p class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $category->name }}
                            </p>
                            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $category->articles_count }}
                                berita</p>

                            <form id="delete-category-form-{{ $category->id }}"
                                action="{{ route('regulasi-institusi.kategori.destroy', $category->id) }}"
                            ...
                            <div class="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
                                <a href="{{ route('regulasi-institusi.kategori.show', $category->slug) }}"
                                ...
                                <a href="{{ route('regulasi-institusi.kategori.edit', $category->id) }}"
    ...
    <div id="deleteCategoryModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/50 px-4"
        aria-hidden="true">
        <div
            class="w-full rounded-2xl border border-slate-200 bg-white p-6 shadow-xl dark:border-slate-800 dark:bg-slate-900 md:w-[28rem]">
            <h2 class="text-lg font-bold text-slate-900 dark:text-slate-100">Hapus Kategori</h2>
            <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">
                Kategori <span id="deleteCategoryName" class="font-semibold text-slate-900 dark:text-slate-100"></span>
                akan dihapus beserta artikelnya.
            </p>

            <div class="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                <button type="button" id="cancelDeleteCategoryButton"
                    class="inline-flex items-center justify-center rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
                    Batal
                </button>
                <button type="button" id="confirmDeleteCategoryButton"
                    class="inline-flex items-center justify-center rounded-xl bg-rose-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-rose-700 disabled:opacity-70">
                    Hapus
                </button>
            </div>
        </div>
